...
# Nomad
$ vagrant ssh node-1 -c "nomad server members -detailed"
$ vagrant ssh node-1 -c "nomad node-status"
$ vagrant ssh node-1 -c "nomad job run <path/to/jobspec.nomad>"
$ vagrant ssh node-1 -c "nomad deployment list"
$ vagrant ssh node-1 -c "nomad job status <Job_ID>"
$ vagrant ssh node-1 -c "nomad job stop <Job_ID>"
...
** 2. Stack Deployment
| # | Action               | Command     |
| 1 | Change the IP addresses of the =hosts= file accordingly with what you set in the =Vagrantfile= | =$ cat /ansible/inventory/hosts= |
